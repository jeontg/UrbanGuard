"""traffic-service(``service/traffic_service.py``) — 인자 없는 GET API가
터지지 않는가.

⚠️ 2026-08-31 신설 — API 게이트웨이 Phase 4로 교통위험 API가
...
def test_api_가_터지지_않는다(client, db_schema, path):
    r = client.get(path)
    # /api/traffic/roi-flow/propose는 camera_id 쿼리파라미터가 필수라
    # 인자 없이 부르면 422다 — 500만 아니면 이 시험의 목적(안 터짐)엔
    # 문제 없다.
