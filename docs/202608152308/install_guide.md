...
```bash
cd D:\dev-PoC\UrbanGuard; .\.venv\Scripts\urbanguard-bootstrap.exe --login-id admin --name 관리자 --dept 정보통신과
```
...
> ### ⚠️ 2026-08-15 이전 판을 보고 설치하셨다면
> `urbanguard-bootstrap` 이라는 이름이 문서에는 있었지만 **실제로는 등록돼
> 있지 않아 실행되지 않았습니다.** 이번 판에서 등록했습니다. 이미 설치된
> 환경이라면 `pip install -e .` 를 다시 한 번 실행해야 명령이 생깁니다.

### 2-6-B. ★ 비밀번호를 잃었을 때 (2026-08-15 신설)

**시스템관리자 계정의 비밀번호를 모두 잃으면 화면으로는 복구할 수 없습니다.**
서버에서 아래 명령을 씁니다.

```bash
D:\dev-PoC\UrbanGuard\.venv\Scripts\urbanguard-passwd.exe --list
```

```bash
D:\dev-PoC\UrbanGuard\.venv\Scripts\urbanguard-passwd.exe --login-id admin
```

| 옵션 | 하는 일 |
|---|---|
| `--list` | 시스템관리자 계정 목록·상태 확인 (**아무것도 바꾸지 않음**) |
| `--login-id <아이디>` | 비밀번호 초기화 (잠금도 함께 해제) |
| `--unlock` | **비밀번호는 그대로 두고** 잠금만 해제 |
| `--from-stdin` | 새 비밀번호를 표준입력에서 읽음 (명령 이력에 안 남음) |
| `--yes` | 확인 질문 건너뛰기 (자동 실행용) |

**비밀번호를 인자로 받지 않습니다** — PowerShell·bash 이력에 평문으로 남기
때문입니다. 초기화 결과로 나온 임시 비밀번호는 **한 번만 출력**되며, 로그인하면
비밀번호 변경 화면이 먼저 나옵니다.

> 이 명령의 방벽은 **서버 접근 권한뿐**입니다. 아무도 로그인할 수 없는 상황을
> 푸는 도구라 그럴 수밖에 없으며, 그래서 **사용 사실이 감사 로그(S-91)에
> 「서버 CLI」로 기록**됩니다.
...
### 3-1-B. ★ 감시 기동 — 죽어도 되살아나게 (2026-08-13 신규)

**시범운영·상시 운영에서는 이쪽을 쓰십시오.**

```bash
cd D:\dev-PoC\UrbanGuard; .venv\Scripts\python.exe scripts\serve.py
```

| 옵션 | 기본값 | 설명 |
|---|---|---|
| `--host` | 127.0.0.1 | 외부 접속을 받으려면 `0.0.0.0` |
| `--port` | 8000 | |
| `--max-restarts` | 20 | 연속 재기동 상한(0=무제한) |

**왜 필요한가** — 2026-08-12 밤, 부산 CCTV 호스트가 풀리지 않는 동안 스트림
재접속이 반복되면서 서비스가 **세그폴트로 죽었고 밤새 내려가 있었습니다.**
세그폴트는 파이썬에서 잡을 수 없어(예외조차 오지 않음) 바깥에서 지켜보는
방법뿐입니다.

- 비정상 종료를 감지하면 5초 뒤 재기동(반복되면 최대 120초까지 간격 증가)
- 5분 이상 정상 가동했다면 「일시적 원인」으로 보고 간격을 되돌림
- **Ctrl+C 는 운영자 의도**이므로 되살리지 않고 함께 종료
- 기록: `data\logs\serve.log`

> ### ⚠️ 서버 재부팅은 넘지 못합니다
> 이 스크립트도 함께 죽습니다. **운영 납품에서는 Windows 서비스(NSSM 등)나
> 작업 스케줄러로 등록**해 부팅 시 자동 기동되게 해야 합니다. 현재는 그때까지의
> 대안입니다.
...
| 8 | **서비스 재시작** | ⚠️ **인파·침수만** 필요 (노면은 즉시 반영) |
...
> **⚠️ 8번을 빠뜨리면 인파·침수 상시 탐지가 켜지지 않습니다.** 화면에도 안내가
> 뜹니다. **노면은 저장 즉시 반영**되며, 진행 중이던 관측이 끝날 때까지 잠깐
> 「반영 대기」로 표시됩니다(2026-08-13 개선).
...
5회 실패 시 10분간 잠깁니다. 푸는 방법이 세 가지입니다.

| 상황 | 방법 |
|---|---|
| 비밀번호는 기억함 (오타로 잠김) | S-90에서 **「잠금 해제」** — 비밀번호는 그대로 |
| 비밀번호를 잊음 | S-90에서 **「비밀번호 초기화」** |
| **아무도 로그인할 수 없음** | 서버에서 `urbanguard-passwd --login-id <아이디>` (2-6-B절) |

**잠금 해제와 초기화는 다릅니다.** 잠김의 대부분은 단순 오타인데 초기화하면
멀쩡한 비밀번호까지 버리게 됩니다.

### 5-4-B. ★ 오류 원인을 모르겠을 때 (2026-08-15 신설)

**관리자 › 오류 관리(S-92)** 를 먼저 보십시오. 화면·API 오류뿐 아니라
**상시 탐지 워처의 오류까지** 모입니다. 코드를 펼치면 원인과 조치 방법이
바로 나옵니다.

| 자주 보게 될 코드 | 뜻 |
|---|---|
| `UG-CCTV-002` | 스트림 주소 DNS 조회 실패 — 관제망·DNS 확인 |
| `UG-DB-001` | 데이터베이스 연결 실패 — PostgreSQL 기동·포트 확인 |
| `UG-AI-001` | 모델 파일 없음 — **학습 데이터를 정리하며 모델까지 옮기는 사고가 흔함** |
| `UG-AUTH-005` | 권한 규칙 미등록 경로 차단 — 개발 부서 전달 필요 |
| `UG-SYS-001` | 분류되지 않은 오류 — 상세를 그대로 개발 부서에 전달 |

> **⚠️ 오류 화면이 비어 있다고 「오류가 없다」로 읽으면 안 됩니다.**
> DB에 연결하지 못하는 동안에는 오류 자체도 기록되지 않습니다.
