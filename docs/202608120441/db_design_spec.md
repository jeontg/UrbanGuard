...
> `interface_spec.md`(인터페이스) · `../ui_design_spec.md`(화면 설계)
...
| # | 컬럼 | 타입 | NULL | 설명 |
|---|---|---|---|---|
| 1 | `key` | varchar(64) **PK** | N | `org_name` / `solution_name` / `logo_file` / `board_bg` / `facility_mode` |
| 2 | `value` | text | N | |
...
### 13-7. 상시 탐지 변경이 즉시 반영되지 않습니다

`camera_domains.continuous` 를 바꿔도 **서비스를 재시작해야** 적용됩니다.
분석 스레드를 실행 중에 교체하면 추적 상태가 어긋나기 때문입니다.

관제 중 재시작은 부담이므로, 향후 **탐지 대상 변경을 무중단 반영**하려면
지점별 워커를 개별 기동·정지할 수 있는 구조로 바꿔야 합니다. 현재는 화면에
재시작 안내를 띄우는 것으로 갈음하고 있습니다.
